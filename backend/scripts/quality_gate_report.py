...
def _build_runtime_contract_summary(checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "child_executor_dispatch_retry_scheduler_binding_gate_coverage": (
            _build_child_executor_dispatch_retry_scheduler_binding_gate_coverage(
                child_executor_dispatch_retry_scheduler_binding_gate_check
            )
        ),
        "child_executor_dispatch_retry_scheduler_execution_authorization_coverage": (
            _build_child_executor_dispatch_retry_scheduler_execution_authorization_coverage(
...
def _build_child_executor_dispatch_retry_scheduler_binding_gate_coverage(check: dict[str, Any]) -> dict[str, Any]:
    contract_version = str(check.get("contract_version") or "").strip()
    default_status = str(check.get("default_status") or "").strip()
    default_handoff_ready = bool(check.get("default_handoff_ready"))
    default_binding_ready = bool(check.get("default_binding_ready"))
    default_missing_sections = _normalize_string_list(check.get("default_missing_sections"))
    default_will_schedule_retry = bool(check.get("default_will_schedule_retry"))
    ready_status = str(check.get("ready_status") or "").strip()
    ready_binding_ready = bool(check.get("ready_binding_ready"))
    ready_binding_source = str(check.get("ready_binding_source") or "").strip()
    ready_will_schedule_retry = bool(check.get("ready_will_schedule_retry"))
    production_blocked_status = str(check.get("production_blocked_status") or "").strip()
    production_blocked_sections = _normalize_string_list(
        check.get("production_blocked_sections")
    )
    missing_audit_idempotency_status = str(
        check.get("missing_audit_idempotency_status") or ""
    ).strip()
    missing_audit_idempotency_sections = _normalize_string_list(
    ...
    missing_worker_attempts_sections = _normalize_string_list(
...
def _build_child_executor_dispatch_retry_scheduler_execution_authorization_coverage(check: dict[str, Any]) -> dict[str, Any]:
    contract_version = str(check.get("contract_version") or "").strip()
    default_status = str(check.get("default_status") or "").strip()
    default_binding_gate_ready = bool(check.get("default_binding_gate_ready"))
    default_authorization_ready = bool(check.get("default_authorization_ready"))
    default_missing_sections = _normalize_string_list(check.get("default_missing_sections"))
    default_will_schedule_retry = bool(check.get("default_will_schedule_retry"))
    default_retry_scheduled = bool(check.get("default_retry_scheduled"))
    ready_status = str(check.get("ready_status") or "").strip()
    ready_authorization_ready = bool(check.get("ready_authorization_ready"))
    ready_authorization_source = str(check.get("ready_authorization_source") or "").strip()
    ready_will_schedule_retry = bool(check.get("ready_will_schedule_retry"))
    ...
    missing_durable_status = str(check.get("missing_durable_status") or "").strip()
    missing_durable_sections = _normalize_string_list(
