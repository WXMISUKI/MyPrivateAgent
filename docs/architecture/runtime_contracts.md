...
- `runtime_contract_smoke.py` 必须输出 `child_executor_dispatch_contract` check，证明默认 dispatch boundary 仍保持 blocked、`dispatch_ready = false`、`will_dispatch = false`，且 backend dispatch readiness blocker 机器可读；该 check 还必须覆盖默认 handoff blocked、opt-in sandbox attempt envelope-ready、unsafe payload guard fail-closed 三类 evidence。`quality_gate_report.py` 与 Runtime Contract Gate 必须从该 check 推导 `runtime_contract_summary.child_executor_dispatch_coverage`，缺失、非对象或证据不完整时应 fail-closed 为 `dispatch_smoke = false`。
...
- `runtime_contract_smoke.py` 必须输出 `child_executor_sandbox_backend` check，证明 sandbox worker backend adapter 的 ready contract、missing guard fail-closed、unsafe payload fail-closed、compact attempt envelope 与 backend invocation count 已进入门禁覆盖；`quality_gate_report.py`、Runtime Contract Gate 与 Snapshot 必须从该 check 推导并守护 `runtime_contract_summary.child_executor_sandbox_backend_coverage.sandbox_backend_smoke`。该 coverage 只说明 adapter gate evidence 健康，不表示默认启用 worker、queue、sandbox runtime 或远端 executor。
...
- `child_executor_dispatch_contract` 当前是 child executor 真正执行前的最终 side-effect-free 边界。它读取 promotion gate、execution prerequisites 与 backend registry，默认保持 `overall_status = blocked`、`dispatch_ready = false`、`will_dispatch = false`，避免消费方把 `promotion_gate.allowed = true` 误解释为已经可以启动真实 worker。其嵌套 `child_executor_dispatch_attempt_handoff` 只验证 dispatch attempt envelope handoff 是否可构造；即使 opt-in sandbox handoff ready，`will_dispatch` 仍为 `false`，真实调用仍必须经过显式 enabled 的 `ChildExecutorDispatcher` 与注入 backend adapter。
...
- `child_executor_sandbox_worker_backend` 当前定义真实 child executor dispatch 前的 sandbox backend adapter contract seam：backend 必须暴露 adapter contract、sandbox/resource/audit/idempotency guard evidence，并返回 compact dispatch attempt envelope。缺 guard、unsafe payload、malformed attempt output 或 adapter exception 都必须 fail-closed；默认 registry 仍保持 relationship-only，不会启动 worker、queue 或 sandbox runtime。
...
- `runtime_contract_gate_degraded.payload.runtime_contract_summary.child_executor_sandbox_backend_coverage` 必须由后端 trace 入口归一化后写入；缺失、非对象或证据不完整时应按 `sandbox_backend_smoke = false` 处理，并纳入 fingerprint / dedupe key，确保 sandbox adapter gate coverage 变化会形成新的治理信号。
...
- `child_executor_dispatch_contract`：side-effect-free dispatch boundary，用于表达当前是否具备真实 child executor dispatch 条件。它不会启动 executor，`will_dispatch` 默认始终为 `false`，并嵌套 `child_executor_dispatch_attempt_handoff` 来说明 sandbox attempt envelope validation、unsafe payload guard、audit/idempotency handoff 证据。真实 dispatcher 必须作为后续显式 opt-in boundary 接入；dispatcher 返回的 `dispatch_result_handoff` 则描述 adapter result 的 compact audit handoff，不代表 parent merge 或 retry execution。
...
- `RuntimeSurfaceService.get_runtime_profile()` 当前已拆出独立 profile assembler；其中模型/提供方聚合已进一步下沉到 `ProviderCatalogBuilder`，后续新增 profile 组装能力时，应优先延续 concern-specific builder 边界，而不是继续把逻辑堆回 service 主方法或 assembler 主流程。
