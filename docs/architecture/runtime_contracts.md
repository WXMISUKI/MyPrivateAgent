...
- `RuntimeSurfaceService.get_runtime_profile()` 当前已拆出独立 profile assembler；顶层 profile shell 位于 `backend/services/runtime_surface_profile_assembler.py::RuntimeSurfaceProfileAssembler`，模型/提供方聚合已进一步下沉到 `ProviderCatalogBuilder`。后续新增 profile 组装能力时，应优先延续 concern-specific builder 边界，而不是继续把逻辑堆回 service 主方法或 assembler 主流程。
