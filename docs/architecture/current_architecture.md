...
- Domain agent grounded answer 现在具备显式 live provider trial：`DomainAgentLiveGroundedAnswerTrialService` 与 `backend/scripts/domain_agent_live_grounded_answer_trial.py` 会读取 agent manifest 的 `rag_sources`，调用外部 provider `/api/rag/retrieve`，并把真实 `evidence_pack` 接入 trial/package/composition 控制面；该入口仍不启用默认 `/api/chat` retrieval injection，不创建 source binding，不执行 GraphRAG，不写 memory/audit/trace。
- MyPrivateAgent 现在具备本地知识 provider 语料试跑：`scripts/export_local_knowledge_provider_corpus_trial.py` 会针对 provider-visible source，例如 `company_profile_2025_trial`，调用外部 provider 的 source catalog、source manifest、RAG retrieve 与 cited answer 合同，并输出 `go / review / blocked` 结果；该入口仍不启用默认 `/api/chat` retrieval injection，不创建 source binding，不写 memory/audit/trace，不启动 provider，也不执行 GraphRAG。
- MyPrivateAgent 现在具备本地业务 RAG 用户闭环收口：`scripts/export_business_rag_user_loop_closure.py` 会读取本地知识 provider 语料试跑与公司简介显式 API smoke 产物，汇总为单一 `go / review / blocked` 结论，用来判断当前 `company_profile_2025_trial` 是否可以进入本地业务问答试用；该入口只读、不重新调用 provider、不接入默认 `/api/chat`、不创建 source binding、不写 memory/audit/trace，也不执行 GraphRAG。
...
- MyPrivateAgent 现在具备本地文档 RAG 操作入口：`POST /api/document-rag/local-trials/readiness` 与 `POST /api/document-rag/local-trials` 会把 readiness 和 upload-to-use loop 包装为本地 operator API，Settings diagnostics 也提供最小操作卡片；该入口仍只接收本地文档路径，不做完整知识库管理，不接入默认 `/api/chat`，不创建 source binding，不写 memory/audit/trace，不启动外部服务，也不执行 GraphRAG。
