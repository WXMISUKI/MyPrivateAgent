...
- Execution Loop 已具备 `ExecutionLoopController` 最小 seam，可驱动 run 经过 planning / generating / tool_calling / waiting_approval / observing / finalizing / done，并写入统一状态事件；同时支持可插拔 model step、tool policy、tool executor、reflector、reviewer 与 fallback handler，形成可审计 model generation / permission / act / reflection / review / degradation gate；model step 在 `generating` 阶段执行，输出归一化为 compact evidence 并写入 `execution_loop_model_step_completed` event；tool policy `denied` 会 fail-closed，不会继续执行工具。
