...
- Provider Ops 控制面已新增 `provider-ops-control-plane-v1`：`/api/provider-ops` 统一暴露 credential/quota/rate-limit/cost/SLA/fallback posture。该层只读解释既有 provider 配置与治理元数据，不暴露 secrets，不驱动路由，不改变 `/api/chat` 行为，也不替代 `provider-service-consumption-v1` 或 `provider-onboarding-catalog-v1`。
...
- 前端设置页的 `ProviderConfigPanel` 已作为独立 provider configuration control plane 收口：它只负责 `/api/providers` 的本地配置读写和显式测试，不暴露原始 API key，不替代 `/api/service-providers`，也不替代 onboarding catalog。
- 前端设置页的 `Provider Failover 看板` 已作为只读 observability contract 收口：它消费 `/api/failover-analytics` 和 runtime profile thresholds，只展示切换率、切换子任务、总切换次数、平均切换次数以及 top provider/model 路径，不改变 planner、scheduler 或 provider 路由。
...
- Coze workflow migration 已新增 Workflow Lab 只读回放面：它只读消费 workflow registry、dependency mapping、acceptance examples 和 replay diff，不是新的执行链，也不改变 `/api/chat`、provider settings 或 capability runtime 的 fail-closed 规则。
- Embedded SDK 恢复消费验收已新增 `embedded-sdk-recovery-acceptance-smoke-v1`：`embedded_sdk_recovery_acceptance_smoke.py` 可生成 durable registry-backed accepted 证据，并固定 memory-only workspace、missing registry binding 两类 blocked 证据。该 gate 只证明显式 SDK recovery consumption readiness，不代表 worker lease、后台自动恢复、分布式 executor、真实 LLM 或默认 `/api/chat` 行为已启用。
...
- Phase II 收束评估已完成（`docs/change/phase-ii-exit-gate-assessment.md`）：四项收束标准均未完全达标（SDK persistence 60%、governance 前端拆分 50%、Runtime Surface assembler 40%、团队判断标准 50%），但 SDK 路径已证明可用。建议采用混合方案：完成 governance 前端拆分和 Runtime Surface assembler 提取后关闭 Phase II，进入 Phase III。
- Governance Timeline 前端拆分已完成：`GovernanceTimelinePanel.vue` CSS 提取到独立文件，组件从 1,208 行降到 884 行（-324 行）。governance 前端拆分进度从 50% 提升到约 70%。
...
1. 深化 Embedded SDK 持久化与恢复能力。
2. 准备 framework adapter authoring checklist / promotion gate，而不是直接接入第二个框架。
3. 拆 Runtime Surface contract assembler。
4. 拆 Governance Timeline。
5. 仅在真实接入方暴露缺口时继续补 provider/domain-agent trial 证据。
