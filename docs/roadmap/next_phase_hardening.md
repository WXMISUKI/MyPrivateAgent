...
- 再下一刀若继续推进 child executor，应优先评估真实 sandbox backend adapter binding 或更窄的 scheduler binding/production gate evidence，不要默认启用 worker 或 retry scheduler。
...
- 评估下一刀是否推进真实 sandbox backend execution seam 或 child dispatch retry scheduler gate；在此之前不要把 adapter contract ready 误读为 production child executor dispatch ready。
