...
Recovery operation records MUST contain audit evidence without copying executable internals, and their contract construction SHOULD live behind a dedicated recovery operation Module rather than inside the SDK orchestration class. When retry evidence is supplied, the operation record MUST preserve compact retry fields without requiring executable internals.
