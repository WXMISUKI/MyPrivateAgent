# runtime-core-terms-model Specification

## Purpose
Define Runtime Core terminology so query, run, child run, trace, audit, artifact, and snapshot references stay consistent across contracts and UI.
## Requirements
...
#### Scenario: Query is not a run alias
- **WHEN** a governance view, read model, or contract needs to identify the lifecycle of a request
- **THEN** it MUST use `query_id` as the lifecycle key
- **AND** it MUST only use `run_id` to refer to one execution instance inside that lifecycle
...
#### Scenario: Artifact is not snapshot_ref
- **WHEN** a contract or UI references a result object
- **THEN** `artifact` MUST refer to the reusable runtime result itself
- **AND** `snapshot_ref` MUST remain the indirection/reference form, not a synonym for the artifact object
