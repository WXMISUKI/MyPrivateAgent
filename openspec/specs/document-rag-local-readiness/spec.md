...
## Purpose
Defines the side-effect-free local readiness check for real document-to-RAG trials. The checker helps an operator confirm OCR, unifiedKnowledgeRAG, source visibility, and provider command prerequisites before running a slow document upload-to-use loop.
