...
## Purpose
Defines the explicit local loop for turning a real uploaded document into a provider-visible RAG source trial. The loop stays as operator tooling: MyPrivateAgent orchestrates document parsing and artifact handoff, while unifiedKnowledgeRAG owns parser artifact ingestion, retrieval, answer citation checks, and source usability.
